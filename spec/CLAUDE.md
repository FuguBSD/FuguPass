...
This directory holds the FuguPass specification.
...
- A rule ID names one requirement inside a unit, as a bold-lead list item, for example
  KEY-DERIVE-3. Rule numbers only append: never renumber, and never reuse a number.
- A plan cites units and rules: `Implements: KEY-DERIVE without KEY-DERIVE-3` and
  `Defers: KEY-BIP85`.
