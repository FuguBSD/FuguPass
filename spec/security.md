...
memory hygiene rules, and the honesty duties of the documentation. The
derivation rules are in [keys.md](keys.md). The accepted limits of the design
are in [OVW-RISKS](overview.md#ovw-risks).

<a id="sec-entropy"></a>

## Entropy policy

- **SEC-ENTROPY-1** — The system RNG must not generate a stored secret.
- **SEC-ENTROPY-2** — A long-term secret must enter from outside, or must derive
  from the master ([KEY-MASTER](keys.md#key-master),
  [KEY-DERIVE](keys.md#key-derive)). A secret from outside is the master or an
  import.
- **SEC-ENTROPY-3** — The device factor must derive from the master
  ([KEY-DEVICE](keys.md#key-device)). The tool must not create a device factor
  from the system RNG.
- **SEC-ENTROPY-4** — Ephemeral request keypairs, IVs, seal nonces, and
  `set_pin` entropy must come from `arc4random(3)`.
- **SEC-ENTROPY-5** — Every salt must derive from the device factor
  ([KEY-PIN](keys.md#key-pin)). A salt is not random.
- **SEC-ENTROPY-6** — The master and `root` must not persist on disk. A one-way
  32-byte check value derived from `root` can persist on disk
  ([KEY-MASTER](keys.md#key-master)).
- **SEC-ENTROPY-7** — A Shamir coefficient must derive from the split secret
  ([KEY-SHARE](keys.md#key-share)). The tool must not draw a coefficient from
  the system RNG.

The scope of the rule keeps the oracle protocol possible. Every oracle request
needs an ephemeral keypair and a fresh IV. Every seal write needs a fresh nonce.
That randomness is ephemeral or public, never a stored secret. The derived
device factor lets the plate regenerate the client keys of every machine.
Revocation from the plate rests on that property (see
...
- **SEC-MEMORY-3** — Every FuguPass C program must hold `RLIMIT_CORE` at zero,
  first in `main()`. The program must read the limit with `getrlimit(2)`, and it
  must call `setrlimit(2)` only when the limit is not zero. A child of the core
  process inherits the zero limit of that process. The execpromises of that
...
- **SEC-MEMORY-5** — The master must exist in memory only inside a ceremony
  ([KEY-MASTER](keys.md#key-master)).
- **SEC-MEMORY-6** — An entry key must exist in memory for seconds only. The
  client must erase an entry key directly after the seal write or the decrypt
  that uses it.

OpenBSD encrypts swap by default, and the base clang hardening applies to every
build. These platform properties support the rules above. They replace none of
them. The interface process holds no secret
([PROG-IFACE](programs.md#prog-iface)), so the erasure rules bind the C
programs.
...
- **SEC-FLOOR-1** — The disk must hold no offline passphrase verifier. An
  attacker who holds every file on the disk cannot test a passphrase guess
  offline ([KEY-PIN](keys.md#key-pin)).
- **SEC-FLOOR-2** — The man pages and the documentation must state the verifier
...
- **SEC-FLOOR-3** — The documentation must state the loss case: the disk plus
...
- **SEC-FLOOR-4** — The documentation must state the middle case. An attacker
...
verifier case (SEC-FLOOR-2) assumes this breach at one oracle, and the loss case
(SEC-FLOOR-3) assumes the same breach at `k` oracles. A breached record of a
retired or removed oracle counts toward the `k`-breach bound until the owner
...
- **SEC-DETECT-1** — The detection story must rest only on the guarantees of the
  FuguOracle specification. The guarantees are one log line per request with the
  outcome class, and a prominent wipe log (FuguOracle SEC-LOGGING-2).
- **SEC-DETECT-2** — The specification and the documentation must not promise
...
- **SEC-DETECT-3** — Detection is manual. No alert mechanism exists. The owner
  must read the oracle logs, or revocation never happens.
- **SEC-DETECT-4** — The documentation must state this limit: malware that
  reveals a few entries per day looks like the owner in the logs.
- **SEC-DETECT-5** — The documentation must ship an operator runbook. The
  ...
  coverage of every possible quorum (SEC-DETECT-6), and revocation
  ([ORC-REVOKE](oracle.md#orc-revoke)).
- **SEC-DETECT-6** — The documentation must state the coverage bound: an
  ...
  possible quorum intersects them: at most `k − 1` live oracles stay unread.
  With the full set of `n` live oracles, the count is `n − k + 1` oracles.
...
- **SEC-CLAIMS-1** — The documentation must not claim coercion resistance.
- **SEC-CLAIMS-2** — The documentation must not claim a delayed reveal, a
  velocity alarm, or an oracle-side freeze. No such oracle operation exists.
- **SEC-CLAIMS-3** — The documentation must not claim any oracle-side rate limit
  beyond the per-record three strikes.
- **SEC-CLAIMS-4** — The specification must not assume any oracle behavior
  beyond protocol v2 as the FuguOracle specification states it (D-03).

The word "freeze" names no oracle operation. The honest term is revocation, and
