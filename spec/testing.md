...
- **QA-HARNESS-1** — One interop harness must run the oracle-client suite
...
- **QA-HARNESS-6** — The harness must start and stop each counterparty with
  ...
  (QA-HARNESS-4).
- **QA-HARNESS-7** — The FuguPass programs run on OpenBSD only, so on an other
...
- **QA-HARNESS-8** — A test of an interactive ceremony must drive the terminal
...
- **QA-MASK-1** — The mask-stability test must send repeated `get_pin` requests
  to an unchanged record and must assert an identical 32-byte answer plaintext
  on every request.
...
- **QA-MASK-6** — The test must give each run a new record store at the
  ...
  [QA-HARNESS](testing.md#qa-harness) starts the counterparty and builds the
  store.
...
one instance. The quorum leg of [QA-HARNESS](testing.md#qa-harness) covers the
composition.
...
- **QA-ANALYSIS-2** — The analysis must cover key reuse across reveals, the KDF
  of the mask, and the wrap XOR of a share.
...
purpose ([KEY-MASK](keys.md#key-mask)). [QA-MASK](testing.md#qa-mask) pins the
behavior by test, and the analysis judges the construction that FuguPass builds
over that behavior.
...
- **QA-CALIBRATE-1** — A calibration must set the default bcrypt_pbkdf round
...
- **QA-CALIBRATE-4** — The latency measurement of QA-CALIBRATE-1 must run on
  ...
  gives a false round count. The scaling check of QA-CALIBRATE-3 can run in a
...
- **QA-KAT-4** — SeedQR and CompactSeedQR vectors, for 12 words and for 24
  words, must match the SeedSigner specification
  ([CLI-SCAN](programs.md#cli-scan)).
