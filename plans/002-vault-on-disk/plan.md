...
KEY-ENTRY without KEY-ENTRY-1. Implements: SEC-ENTROPY without SEC-ENTROPY-4.
Defers: SEC-MEMORY, ENTRY-POOL, ENTRY-SHADOW, KEY-MASK.
...
the scan helper of plan 012. This plan seals the entry file under `K_e`
(KEY-ENTRY-3), and plan 006 reserves the slots of KEY-ENTRY-1. It draws the seal
nonce of SEC-ENTROPY-4 with `arc4random(3)`. Plan 004 draws the ephemeral
keypair and the IV, and plan 005 draws the `set_pin` entropy.
...
- TEST-KAT reads `partial` with TEST-KAT-3 as the absent rule.
